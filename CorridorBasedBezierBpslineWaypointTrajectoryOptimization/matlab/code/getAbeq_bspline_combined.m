function [Aeq, beq] = getAbeq_bspline_combined(n, p, start_cond_xy, end_cond_xy, init_type)
...
Aeq_temp = [Aeq_start;Aeq_end];
Aeq = zeros(4*p,2*(n+1));
Aeq(1:2*p,1:n+1) = Aeq_temp;
Aeq(2*p+1:4*p,n+2:2*(n+1)) = Aeq_temp;
beq = [start_cond_xy(:,1);end_cond_xy(:,1);start_cond_xy(:,2);end_cond_xy(:,2)];    
