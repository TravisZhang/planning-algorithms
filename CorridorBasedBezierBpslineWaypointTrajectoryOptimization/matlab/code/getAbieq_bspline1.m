...
c_num_end = 1;
more_num_start = p-c_num_start; % more inequal constraint num
more_num_end = p-c_num_end; % more inequal constraint num
Aieq_p_upper = eye(n+1);
Aieq_p_upper = Aieq_p_upper(p+1-more_num_start:n-p+1+more_num_end,:);
Aieq_p_lower = -eye(n+1);
Aieq_p_lower = Aieq_p_lower(p+1-more_num_start:n-p+1+more_num_end,:);
Aieq_p = [Aieq_p_upper;Aieq_p_lower];
bieq_p = zeros((n+1-2*p+more_num_start+more_num_end)*2,1);
for i = 1:n+1-2*p+more_num_start+more_num_end
    bieq_p(i) = corridor_range(i+p-more_num_start,2); % max limit
    bieq_p(i+n+1-2*p+more_num_start+more_num_end) = -corridor_range(i+p-more_num_start,1); % min limit
end
