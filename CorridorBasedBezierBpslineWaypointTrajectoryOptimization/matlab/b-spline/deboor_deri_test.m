...
%% generate transition path by x = f(t) & y = f(t)
    ...
    % test bspline qp optimization for transition(maybe not a good idea
    % because transition will depend on time t
%     init_type = 2;
%     n_temp = 7;
%     c_pts_temp = zeros(n+1,2);
%     p_temp = 4;
%     start_cond_xy = zeros(p_temp,2);
%     end_cond_xy = start_cond_xy;
%     conds = [px0 vx0 ax0 pxf vxf axf py0 vy0 ay0 pyf vyf ayf];
%     for i = 1:2
%         [c_pts_opt,xy_limits] = GetOptBsplineCpts(c_pts_temp,p_temp,start_cond_xy,end_cond_xy,v_max,a_max,init_type);
%     end
