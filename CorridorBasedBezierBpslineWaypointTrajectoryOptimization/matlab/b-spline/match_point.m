function [pos_xy,vel_xy,acc_xy,jerk_xy] = match_point(c_pts,degree,dense,knot_type,current_index)
n = max(size(c_pts))-1;
ns = n-degree+1; % number of segments
current_index = min(max(current_index,1),ns*dense);
% compute knots
...
% assign pos vel acc jerk
pos_xy = pts(current_index,:);
vel_xy = d_pts(current_index,:);
acc_xy = dd_pts(current_index,:);
jerk_xy = ddd_pts(current_index,:);
