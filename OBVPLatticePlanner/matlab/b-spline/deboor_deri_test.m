...
% first get start & end points
    ...
    kf = kappa_set(end_idx);
    dkf = dkappa_set(end_idx);
    % transform end point to start point coord frame
    [xfl,yfl] = global2local(xs,ys,thetas,xf,yf);
    thetafl = thetaf - thetas;
    % calculate transition traj
    py0 = 0;
    vy0 = 0;
    ay0 = ks*(1+vy0^2)^(1.5);
    jy0 = (dks*(1+vy0^2)^4+3*vy0*ay0^2*sqrt(1+vy0^2))/(sqrt(1+vy0^2))^2;
    pyf = yfl;
    vyf = tan(thetafl);
    ayf = kf*(1+vyf^2)^(1.5);    
    jyf = (dkf*(1+vyf^2)^4+3*vyf*ayf^2*sqrt(1+vyf^2))/(sqrt(1+vyf^2))^2;
